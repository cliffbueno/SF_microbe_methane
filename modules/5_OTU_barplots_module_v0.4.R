...
site_colbar = function(Meta_iTag, sample_var, color_var, color_table, order_var, plot){
    ...
    # Make site text label for top of plot - 1 site name for beginning of each color
    # Could make for loop, doing this fast
    colbar_text <- c(levels(samp_info_colorO[,color_var])[1], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[1]) - 1),
                     levels(samp_info_colorO[,color_var])[2], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[2]) - 1),
                     levels(samp_info_colorO[,color_var])[3], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[3]) - 1),
                     levels(samp_info_colorO[,color_var])[4], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[4]) - 1),
                     levels(samp_info_colorO[,color_var])[5], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[5]) - 1),
                     levels(samp_info_colorO[,color_var])[6], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[6]) - 1),
                     levels(samp_info_colorO[,color_var])[7], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[7]) - 1),
                     levels(samp_info_colorO[,color_var])[8], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[8]) - 1),
                     levels(samp_info_colorO[,color_var])[9], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[9]) - 1),
                     levels(samp_info_colorO[,color_var])[10], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[10]) - 1),
                     levels(samp_info_colorO[,color_var])[11], rep("", sum(samp_info_colorO[,color_var] == levels(samp_info_colorO[,color_var])[11]) - 1))
      
    # Make barplot
    p <- ggplot(samp_info_colorO, aes(x=Sample, y=order_var, fill=Group)) +                     # Plot sample by group
            geom_bar(stat='identity',  width = 1, alpha = 0.8) + 
            scale_fill_manual(values = c(site_colU)) +   # Manual color fill
            scale_x_discrete(expand = c(0, 0), position = "top", labels = colbar_text) + # Add text label
            theme(legend.position = "none", # Remove legend
                  axis.title.x=element_blank(),                         
                  axis.text.x.top = element_text(size = 4, angle = 30, hjust = 0, vjust = 0, margin = margin(c(0,0,-2.5,0))), 
                  axis.ticks.x=element_blank()) +                     # Remove all x-axis labs
...
Taxon_bar_newC = function(otu_agg, color_set, min_abund=0.005, color_no=9, output_palette=F, ylab="CPM") {
    ...
    otu_agg2_melt$variable <- as.character(otu_agg2_melt$variable)
...
abund_loc_barplot = function(otu_tab, agg_var, Meta_iTag, order_var, plot="", colors, color_set, relab="CPM"){    # Revised version 0.2.6    
    ...
    bar_plotNL <- bar_plot + scale_y_continuous(position = "right") + # put axis on right side  
                                theme(legend.position="none",
                                      plot.margin = unit(c(-0.3, 0.1, 0, -0.2), "cm"),
                                      axis.title.y.right = element_text(size = 10, angle = 90, margin = margin(t = 0, r = 0, b = 0, l = -5)))   # drop legend from ggplot, rotate y axis title, min. margins    
    ...
    pgl <- plot_grid(legend, pg, rel_widths = c(3,8), align = "h", ncol = 2, axis = "l") # add legend to grid, better alignment later...align="h", ncol=2, 
...
# Barplot function specifically for Figure S11
abund_loc_barplot = function(otu_tab, agg_var, Meta_iTag, order_var, plot="", colors, color_set, relab="CPM"){    # Revised version 0.2.6    
    ...
    bar_plot <- recolor_barplot(otu_S, colors, ylab=y_lab) + # reuse existing palette 
                theme(legend.title = element_text(size = 8, margin = margin(t = 0, r = 0, b = 0, l = 0)),
                      ...
                      legend.key.width = unit(0.35, "cm"),)
    bar_plotNL <- bar_plot + 
                theme(legend.position="none")   # drop legend from ggplot, rotate y axis title, min. margins    
